...
void updateFileList(std::vector<const char*>* p_items,unsigned int* p_item,const char* dir,std::vector<const char*> file_ext,const int file_size=-1){
	...
	//update list
	if (!std::filesystem::is_directory(dir)) std::filesystem::create_directories(dir);//create dir if does not exists
	else{
	...
	if (ImGui::BeginTabBar("MenuTabBar", ImGuiTabBarFlags_None)){
		...
		if (ImGui::BeginTabItem("Emulation")){
			...
			if (disable_load_memory) ImGui::BeginDisabled();
				...
				if (rom==0) ImGui::PushStyleColor(ImGuiCol_Text,ImVec4(1,0,0,1));
				bool rom_combo_state=ImGui::BeginCombo("##rom", roms[rom], 0);
				if (rom==0) ImGui::PopStyleColor();
				if (rom_combo_state!=rom_combo_state_previous){
					updateFileList(&roms,&rom,romDir,romExt,EROM_SIZE);
					
					if(!rom_combo_state){
						if (rom==0) unloadM(p_params->p_gState->p_thread_mutex,&(p_params->p_gState->erom));
						else loadROM(p_params->p_gState->p_thread_mutex,&(p_params->p_gState->erom),roms[rom]);
						//sendCommandMEM(p_mb_circuit,LOAD_EROM,(rom==0)?NULL:roms[rom]);
					}
					/*thread_message ms;
					ms.cmd=LOAD_EROM;
					if (rom==0){
				...
				if (item==0) ImGui::PushStyleColor(ImGuiCol_Text,ImVec4(1,0.5,0,1));
				bool profile_combo_state=ImGui::BeginCombo("##profile", items[item], 0);
				if (item==0) ImGui::PopStyleColor();
				if (profile_combo_state!=profile_combo_state_previous){
					...
					if(!profile_combo_state){
						if (item==0) unloadM(p_params->p_gState->p_thread_mutex,&(p_params->p_gState->eram));
						else loadRAM(p_params->p_gState->p_thread_mutex,&(p_params->p_gState->eram),items[item]);
				...
				if(profile_combo_state){
					for (unsigned int i=0;i<items.size();i++){
						if(i!=item){
							if (i==0) ImGui::PushStyleColor(ImGuiCol_Text,ImVec4(1,0.5,0,1));
				...
				if (ImGui::Button("Supprimer")) {
					...
					unloadM(p_params->p_gState->p_thread_mutex,&(p_params->p_gState->eram));
			...
			ImGui::TextColored(ImVec4(1,0,0,1), "/!\\ ne pas utiliser un même profil pour 2 ROM différentes");
		...
		if (ImGui::BeginTabItem("À propos")){
			...
			ImGui::Text(p_params->info.programmers);
