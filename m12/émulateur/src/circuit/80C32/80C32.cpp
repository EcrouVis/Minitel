...
unsigned char m80C32::getR(unsigned char r){
	return (this->getSFRByteIn(this->PSW)&0x18)|(r&7);
}
...
void m80C32::Reset(){
	...
	//reset Tx/Rx output level
	this->P3_out_alt=0xFF;
	this->sendP3(this->getSFRByteIn(this->P3));
...
void m80C32::execInstruction(){
	...
	switch (this->instruction[0]){
		...
		case 0x10://jbc
			a=this->getBitDirectAddress(this->instruction[1]);
			d=this->JBC(this->getDirectByteOut(a),this->getBitMask(this->instruction[1]),(signed char)this->instruction[2]);
	...
	this->debug_signal_alu_after_exec();
}
